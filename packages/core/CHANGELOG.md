...
- b26fa9a: Advisories MCPA-2026-0091/0092: LudusMCP read_range_config SSRF (CVE-2026-19367) and insert_creds_range_config path traversal (CVE-2026-19366), both verified in the published ludus-mcp@1.0.24 tarball (104 → 106 bundled advisories).

## 0.67.32

### Patch Changes

- 4d18a26: Fresh-corpus precision: AG-RC-001 grades curl|sh values in enclosing `examples:` lists of data files low; AG-CL-001 skips do-not/repeated-run/kebab placeholder-shaped values and treats `_selftest.*` files as test paths; AG-SS-001 treats `.selfcheck.*` files as test paths and recognizes threat-intel IOC-scanner headers as defensive context.
...
- 8c2620d: AG-RC-001/AG-SK-001 precision: mask data heredocs (usage banners) while keeping rendered-script heredocs live, exclude `| node -e`/`| python -c` inline-program pipes (stdin is data), grade yaml/toml curl|sh fixtures under test paths low, and treat `<tag>` usage metavariables after a command word as template notation.
...
- 8880fcb: AG-SK-001 grades injection phrases cited in defensive prose as low: reported speech ("emails that ask you to ignore previous instructions"), parenthetical pattern lists ("scans for (ignore previous instructions, hidden text, etc.)"), and markup parameter content (`<forbidden>conversation_history</forbidden>`) describe the attack rather than mounting it. A cited example cannot mask a real injection elsewhere in the file.

## 0.67.20

### Patch Changes

- ede438f: Other CI systems' pipeline configs (`.gitlab-ci.yml`, `.circleci/config.yml`, `azure-pipelines*.yml`, `.buildkite/*.yml`, `.travis.yml`, `bitbucket-pipelines.yml`, `.drone.yml`, `appveyor.yml`, `cloudbuild.yaml`) are no longer source-scanned — build automation is not MCP server source, matching the existing `.github/workflows` exemption.
