...
Scan every MCP server your clients (Claude Desktop / Claude Code / Cursor / VS Code / Codex / OpenCode / Windsurf / Cline / Gemini CLI / Kiro / Roo Code / Zed / Continue.dev / Amp / Warp / LM Studio / Trae) are configured to use — config paths are discovered automatically:
...
```bash
# Scan the MCP configs on this machine (Claude, Cursor, VS Code, Codex, OpenCode, Windsurf, Cline, Gemini CLI, Kiro, Roo Code, Zed, Continue.dev, Amp, Warp, LM Studio, Trae auto-discovered)
...
steps:
  ...
  - uses: wookat/agentgate/packages/action@v0.24.0
    with:
      command: ci
```
...
Amp, Warp, LM Studio, and Trae on each OS.
...
Windsurf, Cline, Gemini CLI, Kiro, Roo Code, Zed, Continue.dev, Amp, Warp, LM Studio, and Trae, with
