...
```bash
# Scan the MCP configs on this machine (Claude, Cursor, VS Code, Codex, OpenCode, Windsurf, Cline, Gemini CLI, Kiro, Roo Code, Zed, Continue.dev, Amp, Warp, LM Studio, Trae, Qoder, Amazon Q, Qwen Code, Copilot CLI, Junie, Factory Droid, Goose, Crush auto-discovered)
...
steps:
  ...
  - uses: wookat/agentgate/packages/action@v0.55.0
    with:
      command: ci
```
...
GitHub Copilot CLI, JetBrains Junie, Factory Droid, Goose, and Crush on each OS.
