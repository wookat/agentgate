...
Scan every MCP server your clients (Claude Desktop / Claude Code / Cursor / VS Code / Codex / OpenCode / Windsurf / Cline / Gemini CLI / Kiro / Roo Code / Zed / Continue.dev / Amp / Warp / LM Studio) are configured to use — config paths are discovered automatically:
...
```bash
# Scan the MCP configs on this machine (Claude, Cursor, VS Code, Codex, OpenCode, Windsurf, Cline, Gemini CLI, Kiro, Roo Code, Zed, Continue.dev, Amp, Warp, LM Studio auto-discovered)
...
steps:
  ...
  - uses: wookat/agentgate/packages/action@v0.23.1
    with:
      command: ci
```
...
Amp, Warp, and LM Studio on each OS.
...
Windsurf, Cline, Gemini CLI, Kiro, Roo Code, Zed, Continue.dev, Amp, Warp, and LM Studio, with
