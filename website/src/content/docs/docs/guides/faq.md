...
Auto-discovery covers Claude Desktop, Claude Code, Cursor, VS Code, Codex (`config.toml`), OpenCode (`opencode.json`), Windsurf, Cline, Gemini CLI, Kiro, Roo Code, Zed, Continue.dev, Amp, Warp, LM Studio, Trae (project `.trae/mcp.json`), Qoder (`~/.qoder/settings.json`, project `.qoder/settings*.json`), Amazon Q Developer (`~/.aws/amazonq/mcp.json` and `default.json`, named agents in `cli-agents/*.json`, project `.amazonq/mcp.json`/`default.json`/`cli-agents/*.json`), JetBrains Junie (`~/.junie/mcp/mcp.json`, project `.junie/mcp/mcp.json` — shared by the IDE plugin and Junie CLI), Factory Droid (`~/.factory/mcp.json`, project `.factory/mcp.json`), Qwen Code (`~/.qwen/settings.json`, project `.qwen/settings.json`, extensions), GitHub Copilot CLI (`~/.copilot/mcp-config.json`, project `.github/mcp.json`), Goose (`~/.config/goose/config.yaml` extensions, project `recipe.yaml`/`recipe.json` recipe extensions), Crush (`~/.config/crush/crush.json`, project `.crush.json`/`crush.json` — legacy JSON `mcp` map), and the generic `.agents/.mcp.json` convention. Any other client works via `--config path/to/config`.
