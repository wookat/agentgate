...
Auto-discovery covers Claude Desktop, Claude Code, Cursor, VS Code, Codex (`config.toml`), OpenCode (`opencode.json`), Windsurf, Cline, Gemini CLI, Kiro, Roo Code, Zed, Continue.dev, Amp, Warp, LM Studio, Trae (project `.trae/mcp.json`), and the generic `.agents/.mcp.json` convention. Any other client works via `--config path/to/config`.
