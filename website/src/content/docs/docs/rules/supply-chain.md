...
- **Unpinned OpenCode plugins** (`medium`) — npm packages in the `plugin` array of `opencode.json` are auto-installed by Bun and executed at startup; specs without an exact version fetch whatever is latest. Local plugin files (`.opencode/plugins/`, `./…` paths) are loaded from the repo and not flagged here.
