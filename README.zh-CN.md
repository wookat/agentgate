...
steps:
  ...
  - uses: wookat/agentgate/packages/action@v0.17.0
    with:
      command: ci
```
...
支持 Claude Desktop、Claude Code、Cursor、VS Code、Codex、OpenCode、Windsurf、Cline、Gemini CLI、Kiro、Roo Code、Zed、Continue.dev，任何有损转换都会显式告警。也可用独立包 [mcp-agentgate-config-convert](packages/config-convert/)。
